...
def init_train_files(name):
    ...
    for line in f:
        ...
        if sentence:
            if 'PADDING' == sentence:
                sentence = '<s1> %s <e1> may or may not have relation with %s <e2> (padding sentence) </s1>' % (en1_name, en2_name)
            tup_sent = (tup, sentence, en1_name, en2_name, en1_tp, en2_tp)
            all_tup_sent[tup_sent] = True
    ...
    lst_tup_sent = all_tup_sent.keys()
	...
	if (senti+1) % 100 == 0:
        ...
        try:
            ...
            sen_head[senti] = word2id[en1_tp]
        try:
            sen_tail[senti] = word2id[en2_id]
        except KeyError:
            sen_tail[senti] = word2id[en2_tp]
...
def initiate_path(lst_tup_path, parti):        
    ...
    print sen_pos2_g.shape
    print sen_pos1_g.shape
    ...
    if data_type == 'training':
        for i in [0]:
            (instance_triple, instance_scope, train_len, train_label, train_word, train_pos1, train_pos2, train_mask,
             ...
             train_word_ug, train_pos1_ug, train_pos2_ug, instance_scope_ug)= init_train_files("text/train%s" % i)
    ...
    if data_type == 'testing':
        for i in [0]:
            (instance_triple, instance_scope, test_len, test_label, test_word, test_pos1, test_pos2, test_mask,
             ...
             test_word_ug, test_pos1_ug, test_pos2_ug, instance_scope_ug)= init_train_files("text/test%s" % str(i))
